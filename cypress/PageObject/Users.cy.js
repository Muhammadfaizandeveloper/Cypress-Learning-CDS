...
class Users {
    ...
    selelctuserrole() {
       // cy.get(this.userroleSelect).select('4');
        cy.get('select[class="custom-select"]').select('4').should('have.value', '4')
    }
    selectclient() {
        cy.get('select[id="Client_Id"]').select('77', { force: true }).should('have.value', '77')
    }
    activecheckbox() {
    ...
    cy.get('table').find('tr').contains('td', email).parent('tr').find('.right-icon').click();
    cy.on('window:confirm', () => true);
    cy.get('table').find('tr').contains('td', email).should('not.exist');
    }
     
    calculateRowsandCols() {
      cy.get('select[name="user-dataTable_length"]>option').each(($currentrow) => {
        const optionValue = $currentrow.val(); // Assuming you want the value attribute
        cy.get('select[name="user-dataTable_length"]').select(optionValue).should('have.value', optionValue);
        cy.wait(4000);
